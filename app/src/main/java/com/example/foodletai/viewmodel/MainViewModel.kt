...
class MainViewModel(application: Application):AndroidViewModel(application = Application()) {
    ...
    private val _consumedCalories = MutableStateFlow(userData.getConsumedCalories())
    ...
    fun addFoodDescription(foodDescription: String) {
        ...
        if(!model.checkDay(userData)){
            ...
            _consumedCalories.value = userData.getConsumedCalories()
        ...
        if(model.isFoodRelated(foodDescription)) {
            model.askGod(viewModelScope, foodDescription) { result ->
                try {
                    ...
                    lastFood = LastFood(foodDescription,weight, calories, protein, carbs, fat)
    ...
    fun pushLastFood(){
        lastFood?.let { food ->
            userData.setConsumedCalories(consumedCalories.value + (food.calories.replace("g", "").toDoubleOrNull()?.toInt() ?: 0))
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    fun reTry(){
        Log.d("API", "Retrying last food description")
        addFoodDescription(lastFood?.description ?: "")
    }
